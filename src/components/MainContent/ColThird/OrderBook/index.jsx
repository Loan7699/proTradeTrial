    ...
    return (
        <div className='flex-1 m-px overflow-auto bg-[#2f3240]'>
            <div className='flex items-center justify-between leading-[2.5rem] border-b border-solid border-[#444444]'>
                    <div id='asset1' className={`text-xs leading-[2.5rem] ml-10 ] cursor-pointer ${option === 'asset1' && "active"}`} onClick={(e)=> handleOption(e.target.id)}>Lệnh trong ngày</div>
                    <div id='asset2' className={`text-xs leading-[2.5rem] cursor-pointer ${option === 'asset2' && "active"}`}onClick={(e) => handleOption(e.target.id)}>Lệnh điều kiện</div>
                <div className='flex'>
